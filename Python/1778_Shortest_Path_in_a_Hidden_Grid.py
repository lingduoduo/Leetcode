    ...
    def findShortestPath(self, master: "GridMaster") -> int:
        ...
        def dfs(cur, x, y):  # dfs to explore the grid
            ...
            for di, (i, j) in zip(
                ...
                _x, _y = x + i, y + j
                if (_x, _y) not in seen and cur.canMove(
                    di
                ):  # if not reachable and can move
                    cur.move(di)
        ...
        while dq:
            ...
            for i, j in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                ...
                if (_x, _y) not in seen:
                    continue  # only continue BFS on reachable neighbors
                dq.append((step + 1, _x, _y))  # BFS at neighbor positions
                seen.remove(
                    (_x, _y)
                )  # remove from `reachable` once used to avoid repeats
        return -1
