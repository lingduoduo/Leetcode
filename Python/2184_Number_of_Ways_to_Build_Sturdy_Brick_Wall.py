...
class Solution:
    def buildWall(self, height: int, width: int, bricks: List[int]) -> int:
        ...
        d = collections.defaultdict(
            list
        )  # make a adjacency list for {combo: [possible_neighbor_row...]}
        ...
        for i, combo in enumerate(
            ...
            for val in combo[
                :-1
            ]:  # use set `s` to store all join location, we don't care the right edge (hence combo[:-1])
                s.add(cur := cur + val)
            for j, nei in enumerate(combos):
                ...
                for val in nei[:-1]:
                    cur += val
                    if cur in s:
                        break  # if `combo` and its neighbor row `nei` join bricks at `cur`, then this neighbor can't be used at upper row
                else:
                    d[combo].append(nei)
    ...
    class Solution:
        ...
        def buildWall(self, height: int, width: int, bricks: List[int]) -> int:
            ...
            compatible = {
                layout: list(filter(lambda layout2: layout & layout2 == 0, layouts))
            ...
            for i in range(1, height):
                ways = {
                    layout: sum(ways_prev[layout2] for layout2 in compatible[layout])
                    % kMod
                    for layout in layouts
                }
                ways_prev, ways = ways, {}
